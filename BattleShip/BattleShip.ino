...
#define COLOR_OFF   CRGB::Blue   // Color when LED is OFF
#define COLOR_SHIP   CRGB::Green // Color of LED where ships are
...
void updateBothScreens(CRGB leds[9][NUM_LEDS], const bool grid[9][NUM_LEDS], CRGB leds2[9][NUM_LEDS], const bool grid2[9][NUM_LEDS], CRGB activeColor, CRGB inactiveColor) {
  // First clear all the LEDs so the display will be correct
    FastLED.clear();

    // Now update our bottom gridsfor player 1 based on where the ships are
    for (int i = 0; i < 9; ++i) {
        for (int j = 0; j < NUM_LEDS; ++j) {
            leds[i][j] = grid[i][j] ? activeColor : inactiveColor;
        }
    ...
    // Now update our bottom grids for player 2 based on where the ships are
    for (int i = 0; i < 9; ++i) {
        for (int j = 0; j < NUM_LEDS; ++j) {
            leds2[i][j] = grid2[i][j] ? activeColor : inactiveColor;
        }
    }
...
void updateBottomLEDsBasedOnGrid() {
    ...
    if (currentPlayer == PLAYER1 && currentState != SHOOTING) {
        ...
        updateBothScreens(ledsPLAYER1BOTTOM, gridShipPLAYER1, ledsPLAYER2BOTTOM, gridShipPLAYER2, COLOR_SHIP, COLOR_OFF);
    }
...
void updateShootingDisplay() {
    ...
    CRGB hitColor = CRGB::Purple;     // Red for hits
    CRGB missColor = CRGB::Red;  // White for misses
    CRGB backgroundColor = COLOR_OFF;  // Green for background

    // Update the shooting display for Player 1 top screen and Player 2's shots on Player 1's bottom screen
    for (int i = 0; i < 9; ++i) {
      for (int j = 0; j < NUM_LEDS; ++j) {
          // Top Screen (Player 1 sees Player 2's attempts)
          ledsPLAYER1TOP[i][j] = gridHitsPLAYER2[i][j] ? (gridShipPLAYER2[i][j] ? hitColor : missColor) : backgroundColor;
          
          // Bottom Screen (Player 1's ships with Player 2's hits/misses)
          ledsPLAYER2BOTTOM[i][j] = gridShipPLAYER2[i][j] ? (gridHitsPLAYER2[i][j] ? hitColor : COLOR_SHIP) : (gridHitsPLAYER2[i][j] ? missColor : backgroundColor);
      }
    }

    // Update the shooting display for Player 2 top screen and Player 1's shots on Player 2's bottom screen
    for (int i = 0; i < 9; ++i) {
      // Top Screen (Player 2 sees Player 1's attempts)
      for (int j = 0; j < NUM_LEDS; ++j) {           
        ledsPLAYER2TOP[i][j] = gridHitsPLAYER1[i][j] ? (gridShipPLAYER1[i][j] ? hitColor : missColor) : backgroundColor;
          
        // Bottom Screen (Player 2's ships with Player 1's hits/misses)
        ledsPLAYER1BOTTOM[i][j] = gridShipPLAYER1[i][j] ? (gridHitsPLAYER1[i][j] ? hitColor : COLOR_SHIP) : (gridHitsPLAYER1[i][j] ? missColor : backgroundColor);
      }
    } 
...
void handleAttack(int row, int col, CRGB leds[9][NUM_LEDS]) {
    ...
    if (checkAllShipsSunk(enemyGrid, hitGrid)) {
        Serial.println("Game Over! " + String(currentPlayer == PLAYER1 ? "Player 1" : "Player 2") + " wins!");
...
void handleEndGame() { 
  static bool displaySet = false; // Flag to check if the display is already set
    if (!displaySet) {
        // Determine which player is the winner
        bool player1Won = checkAllShipsSunk(gridShipPLAYER2, gridHitsPLAYER2);
        bool player2Won = checkAllShipsSunk(gridShipPLAYER1, gridHitsPLAYER1);

        Serial.print("Player 1 Won: "); Serial.println(player1Won);
        Serial.print("Player 2 Won: "); Serial.println(player2Won);

        // Set the victory and loss colors of the screens
        CRGB winColor = CRGB::Green;
        CRGB loseColor = CRGB::Red;

        FastLED.clear(); // Clear all LEDs once
        ...
        FastLED.show(); // Update the display only once
        displaySet = true; // Set the flag so that this block runs only once
    }
...
void resetGame() {
    ...
    // Reset LED displays
    FastLED.clear(true); // True to also reset all LED data

    // Optionally, set all LEDs to an initial color (e.g., all off or a startup pattern)
    for (int i = 0; i < 9; ++i) {
        for (int j = 0; j < NUM_LEDS; ++j) {
            ledsPLAYER1TOP[i][j] = COLOR_OFF;
    ...
    // Any other initial setup if needed
    Serial.println("Game has been reset. New game can start.");
}
